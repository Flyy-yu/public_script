...
if [ "$1" != "" ]; then
	...
	adb shell 'su -c chmod +x /data/local/tmp/frida-server-12.8.13-android-arm64'

	rm frida-server-12.8.13-android-arm64*

	adb shell 'su -c /data/local/tmp/frida-server-12.8.13-android-arm64 &' || echo 'frida server already running'
